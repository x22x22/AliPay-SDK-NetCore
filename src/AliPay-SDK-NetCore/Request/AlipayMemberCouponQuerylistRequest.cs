...
namespace Aop.Api.Request
{
    ...
    {
        /// <summary>
        ///     红包发放者商户信息，json格式。商户可以传自己的PID，平台商可以传其它商户的PID用于查询指定商户的信息  目前仅支持如下key：    unique：商户唯一标识    unique_type：支持以下1种取值。
        ...
        /// <summary>
        ///     劵所有者买家用户信息，必须是支付宝的用户，json格式。  目前仅支持如下key：    unique：用户唯一标识    unique_type：支持以下1种取值。  UID：用户支付宝账户的唯一ID
        ...
        public IDictionary<string, string> GetParameters()
        {
            ...
            parameters.Add("merchant_info", MerchantInfo);
            parameters.Add("page_no", PageNo);
            parameters.Add("page_size", PageSize);
            parameters.Add("status", Status);
            parameters.Add("user_info", UserInfo);
            return parameters;
        }
