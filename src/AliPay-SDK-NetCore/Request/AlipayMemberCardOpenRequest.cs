...
namespace Aop.Api.Request
{
    ...
    public class AlipayMemberCardOpenRequest : IAopRequest<AlipayMemberCardOpenResponse>
    {
        ...
        /// <summary>
        ///     发卡商户信息，json格式。  目前仅支持如下key：  merchantUniId：商户唯一标识  merchantUniIdType：支持以下3种取值。    LOGON_ID：商户登录ID，邮箱或者手机号格式；
        ...
        /// <summary>
        ///     持卡用户信息，json格式。  目前仅支持如下key：  userUniId：用户唯一标识  userUniIdType：支持以下3种取值。    LOGON_ID：用户登录ID，邮箱或者手机号格式；
        ///     UID：用户支付宝用户号，以2088开头的16位纯数字组成；    BINDING_MOBILE：用户支付宝账号绑定的手机号。
        /// </summary>
        ...
        public IDictionary<string, string> GetParameters()
        {
            ...
            parameters.Add("biz_serial_no", BizSerialNo);
            parameters.Add("card_merchant_info", CardMerchantInfo);
            parameters.Add("card_user_info", CardUserInfo);
            parameters.Add("ext_info", ExtInfo);
            parameters.Add("external_card_no", ExternalCardNo);
            parameters.Add("request_from", RequestFrom);
            return parameters;
        }
