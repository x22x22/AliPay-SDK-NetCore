...
namespace Aop.Api.Util
{
    public static class RSACryptoServiceProviderExtension
    {
                ...
                DERData, (uint) DERData.Length, CRYPT_DECODE_FLAGS.NONE, data, ref dwCertPublicKeyInfoSize))
            {
                    ...
                    DERData, (uint) DERData.Length, CRYPT_DECODE_FLAGS.NONE, data, ref dwCertPublicKeyInfoSize))
                {
                    ...
                    try
                    {
                        ...
                        info = Marshal.PtrToStructure<CERT_PUBLIC_KEY_INFO>(handle.AddrOfPinnedObject());
        ...
        internal static byte[] GetDERFromPEM(string sPEM)
        {
            ...
            if (
                !CryptStringToBinary(sPEM, (uint) sPEM.Length, CRYPT_STRING_FLAGS.CRYPT_STRING_BASE64HEADER, null,
                    ref dwBinarySize, out dwSkip, out dwFlags))
                throw new Win32Exception(Marshal.GetLastWin32Error());

            var decodedData = new byte[dwBinarySize];
            if (
                !CryptStringToBinary(sPEM, (uint) sPEM.Length, CRYPT_STRING_FLAGS.CRYPT_STRING_BASE64HEADER, decodedData,
                    ref dwBinarySize, out dwSkip, out dwFlags))
                throw new Win32Exception(Marshal.GetLastWin32Error());
            return decodedData;
        }
        ...
        internal static extern bool CryptDecodeObject(CRYPT_ENCODING_FLAGS dwCertEncodingType, IntPtr lpszStructType,
            byte[] pbEncoded, uint cbEncoded, CRYPT_DECODE_FLAGS flags, [In] [Out] byte[] pvStructInfo,
