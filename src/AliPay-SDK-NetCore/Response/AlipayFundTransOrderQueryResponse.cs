...
namespace Aop.Api.Response
{
    ...
    public class AlipayFundTransOrderQueryResponse : AopResponse
    {
        ...
        /// <summary>
        ///     转账单据状态。   SUCCESS：成功（同一笔单据多次查询有可能从成功变成退票状态）；   FAIL：失败（具体失败原因请参见error_code以及fail_reason返回值）；   INIT：等待处理；
        ///     DEALING：处理中；   REFUND：退票（具体退票原因请参见fail_reason返回值）；   UNKNOWN：状态未知。
        /// </summary>
        [XmlElement("status")]
