...
namespace Aop.Api
{
    ...
    public class DefaultAopClient : IAopClient
    {
        ...
        public T Execute<T>(IAopRequest<T> request, string accessToken, string appAuthToken) where T : AopResponse
        {
            ...
            CheckResponseSign(request, item.respContent, rsp.IsError, parser, alipayPublicKey, charset, signType,
                keyFromFile);
            ...
            where T : AopResponse
        {
            ...
            {
                var rsaCheckContent = AlipaySignature.RSACheckContent(signItem.SignSourceDate, signItem.Sign,
                    alipayPublicKey, charset, signType, keyFromFile);
                if (!rsaCheckContent)
                    if (!string.IsNullOrEmpty(signItem.SignSourceDate) && signItem.SignSourceDate.Contains("\\/"))
                    {
                        var srouceData = signItem.SignSourceDate.Replace("\\/", "/");
        ...
        /// <returns></returns>
        private AopDictionary SerializeBizModel<T>(AopDictionary requestParams, IAopRequest<T> request)
            where T : AopResponse
        {
            var result = requestParams;
            var isBizContentEmpty = !requestParams.ContainsKey(BIZ_CONTENT) ||
                                    string.IsNullOrEmpty(requestParams[BIZ_CONTENT]);
        ...
        private string Serialize(AopObject obj)
        {
            ...
            var jo = JsonConvert.SerializeObject(obj);
