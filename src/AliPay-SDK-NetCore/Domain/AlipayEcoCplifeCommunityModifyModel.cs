...
{
    /// <summary>
    /// AlipayEcoCplifeCommunityModifyModel Data Structure.
    /// </summary>
    [Serializable]
    public class AlipayEcoCplifeCommunityModifyModel : AopObject
    {
        /// <summary>
        /// 若开发者录入的物业小区需要精确对应地图上多个小区（比如物业系统中的小区包含类似一期二期、或东区西区的组团结构），以便后续线上推广时覆盖到对应小区的住户，可以指定关联的高德地图中住宅、住宿或地名地址等小区相关类型的POI（地图兴趣点）ID列表。  若传入该参数且参数值合法，则该参数的优先级高于传入的地理位置经纬度。    注：最多包含10组poiid。    高德POI ID的获取方法可参考：<a href="http://lbs.amap.com/api/webservice/guide/api/search/">高德WEB服务API文档</a>
        /// </summary>
        ...
        /// <summary>
        /// 小区所在的经纬度列表（注：需要是高德坐标系），每对经纬度用"|"分隔，经度在前，纬度在后，经纬度小数点后不超过6位。    注：若新建的小区覆盖多个片区，最多包含5组经纬度，其中第一组作为主经纬度。    经纬度是小区搜索和用户推广的重要参数，录入时请确保经纬度参数准确。  高德经纬度查询：<a href="http://lbs.amap.com/console/show/picker">人工方式</a>|<a href="http://lbs.amap.com/api/webservice/guide/api/search/">接口方式</a>  坐标系转换：<a href="http://lbs.amap.com/api/webservice/guide/api/convert/">查看接口文档</a>
        /// </summary>
